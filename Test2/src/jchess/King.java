...
public class King extends Piece {
	...
	public ArrayList allMoves() {
		...
		// King is now only able to make horizontal or vertical steps
		
		for (int i = this.square.pozX - 1; i <= this.square.pozX + 1; i++) {
			if (!this.isout(i, this.square.pozY)) {// out of bounds protection
				sq = this.chessboard.squares[i][this.square.pozY];
				if (this.square == sq) {// if we're checking square on which is
										// King
					continue;
				}
				if (this.checkPiece(i, this.square.pozY)) {// if square is empty
					if (isSafe(sq)) {
						list.add(sq);
					}
				}
		...
		for (int y = this.square.pozY - 1; y <= this.square.pozY + 1; y++) {
			if (!this.isout(this.square.pozX, y)) {// out of bounds protection
				sq = this.chessboard.squares[this.square.pozX][y];
				if (this.square == sq) {// if we're checking square on which is
										// King
					continue;
				}
				if (this.checkPiece(this.square.pozX, y)) {// if square is empty
					if (isSafe(sq)) {
						list.add(sq);
					}
				}
		...
		if (!this.wasMotion && !this.isChecked()) {// check if king was not
													// moved before
			...
			if (chessboard.squares[0][this.square.pozY].piece != null
				...
				if (!rook.wasMotion) {
					...
					if (canCastling && this.isSafe(sq) && this.isSafe(sq1)) { // can
																				// do
																				// castling
																				// when
																				// none
			...
			if (chessboard.squares[7][this.square.pozY].piece != null
				...
				if (!rook.wasMotion) {// if king was not moves before and is not
										// checked
	...
	private boolean isSafe(Square s) // A bit confusing code.
	{
		...
		if (s.pozX <= otherKing.square.pozX + 1 && s.pozX >= otherKing.square.pozX - 1
				&& s.pozY <= otherKing.square.pozY + 1 && s.pozY >= otherKing.square.pozY - 1) {
			return false;
		}
		...
		{// System.out.println("go down");
			...
			newY = s.pozY + 1;
			if (!isout(newX, newY)) {
				if (this.chessboard.squares[newX][newY].piece == null) // if on
																		// this
																		// sqhuare
																		// isn't
																		// piece
